...
//  SST Announcer
//
//  Created by Pan Ziyue on 27/11/16.
...
extension String {
    ...
    /// An `NSAttributedString` generated from a HTML `String`, intended for use as a simple decoding function on the Apple Watch
    /// *NOTE: Using this function is processor intensive, do NOT use it on multiple pieces of content at once*
    ...
    /// String representation of "clean" HTML, eliminating "dirty" HTML caused by text to HTML editors like Blogger, as well as bad authors using websites like Blogger causing faulty text to be generated by the editor in Blogger
    var cleanHTML: String {
        var html = self
        //html = html.replacingOccurrences(of: " style=\"[\\s\\S]*?\"", with: "", options: .regularExpression, range: nil)
        html = html.replacingOccurrences(of: " height=\"[\\s\\S]*?\"", with: "", options: .regularExpression, range: nil)
        html = html.replacingOccurrences(of: " width=\"[\\s\\S]*?\"", with: "", options: .regularExpression, range: nil)
        html = html.replacingOccurrences(of: " border=\"[\\s\\S]*?\"", with: "", options: .regularExpression, range: nil)
